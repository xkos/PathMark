...
- GitHub 发布：`.github/workflows/release-extension.yml` 在 main/PR 自动测试、构建并上传保留 14 天的 ZIP；`v*` 标签校验 package、Manifest 和 tag 版本一致后，受保护的 `release` job 使用固定 RSA 密钥生成 CRX3、`SHA256SUMS.txt` 与 GitHub Release。首次正式版本 `v0.1.0` 已构建、签名并发布成功；Chrome Web Store 上架版本已升级为 `0.1.1`，等待创建对应 Release。普通 CI 不读取签名密钥。
...
- 剩余发布动作：创建并验收 `v0.1.1` Release，将 ZIP 与商店材料上传 Chrome Web Store 并提交审核；安全离线备份固定 CRX 签名 PEM；在 Chrome 与 Edge 手工执行一次完整安装验收。Microsoft Edge Add-ons 仍需后续单独上架。
